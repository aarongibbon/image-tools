...
def return_valid_files(file_paths, logger):
    valid_files = []
    for file_path in file_paths:
        ...
        if os.path.getsize(file_path) == 0:
            ...
            continue
        if contains_illegal_pattern(str(absolute_path), logger):
            continue
...
def process(src_root, dest_root, logger, dry_run=False, delete_source=False):
    valid_files = []
    ...
    valid_files = return_valid_files(src_files, logger)
...
if __name__ == '__main__':
    ...
    process(args.source, args.destination, logger, args.dry_run, args.delete_source)
